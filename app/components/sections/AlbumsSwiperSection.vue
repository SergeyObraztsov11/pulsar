...
/**
 * AlbumsSwiperSection — горизонтальный swiper альбомов (free-mode + скелетоны).
...
<template>
  ...
  <section class="flex flex-col gap-6">
    ...
    <div
      v-else
      class="albums-shelf w-full"
      :class="{
        'albums-shelf--fade-left': !isBeginning,
        'albums-shelf--fade-right': !isEnd,
      }"
    >
      <ClientOnly>
        <Swiper
          ...
          class="albums-shelf__slider w-full"
          @swiper="onSwiper"
        ...
        >
          <SwiperSlide
            ...
            class="w-48! shrink-0"
          >
            <AlbumCard :album="album" :interactive="props.interactive" />
          </SwiperSlide>
        </Swiper>
...
.albums-shelf__slider :deep(.swiper-wrapper) {
  display: flex;
  flex-direction: row;
}
</style>
